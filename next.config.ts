...
const nextConfig: NextConfig = {
  /**
   * A self-contained `.next/standalone` build (Docker/App Runner, `docs/deploy-plan.md`).
   *
   * Next traces every import reachable from a route and copies just those `node_modules`
   * into `.next/standalone`, alongside a `server.js` that needs nothing else installed. That
   * is what makes the runtime image small and lets it run on plain `node` rather than `bun` —
   * the trace is static-file copying, not a package-manager-specific step, so it does not
   * care that the repo was built with Bun.
   *
   * Harmless locally: `bun run dev` and `bun run build && bun run start` both ignore this and
   * use the normal `.next` output. It only changes what `next build` additionally emits.
   */
  output: "standalone",
